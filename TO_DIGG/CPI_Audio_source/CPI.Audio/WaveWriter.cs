...
namespace CPI.Audio
{
    ...
    public abstract class WaveWriter : IDisposable
    {
        ...
        /// </param>
        protected WaveWriter(Stream output, int sampleRate, short bitsPerSample, bool stereo, bool closeUnderlyingStream)
        {
            if (output == null)
        ...
        public long CurrentSample
        {
            get
            {
                ...
                return (_writer.BaseStream.Position - WaveHeader.HeaderSize - _initialStreamPosition) / _header.BytesPerSample;
            }
            set
            {
                ThrowIfDisposed();
                ...
                if (value < 0)
                    throw new ArgumentOutOfRangeException("value", "Sample Number must be greater than or equal to zero.");
